...
- (void)loadUIConfig {
	...
	for (NSInteger i = 0; i < self.shareBtnTitleArray.count; i++) {
		...
		CGFloat x = self.bgView.bounds.size.width / ItemIndex * ( i % ItemIndex);
        CGFloat y = _protext.length == 0 ? 0 : LABEL_HEIGHT + (i / ItemIndex) * LINE_HEIGHT;
		CGFloat w = self.bgView.bounds.size.width / ItemIndex;
		CGFloat h = 70;
...
- (UIView *)bgView {
	...
	if (_bgView == nil) {
		...
		NSInteger index;
		if (_shareBtnTitleArray.count % ItemIndex == 0) {
			
			index = _shareBtnTitleArray.count / ItemIndex;
		} else {
			
			index = _shareBtnTitleArray.count / ItemIndex + 1;
		}
...
- (UIButton *)cancelBtn {
	...
	if (_cancelBtn == nil) {
		...
		[_cancelBtn setTitleColor:[UIColor colorWithHex:0x333333] forState:UIControlStateNormal];
		//	点击按钮，取消，收起面板，移除视图
