...
- (UIView *)topView {
    if (!_topView) {
        _topView =[[UIView alloc]initWithFrame:CGRectMake(0, 0, self.alertView.frame.size.width, kTopViewHeight + 5)];
...
//        [_leftBtn setTitle:@"取消" forState:UIControlStateNormal];
        [_leftBtn setImage:IMAGE_NAMED(@"knb_icon_cancle") forState:UIControlStateNormal];
...
//        [_rightBtn setTitle:@"确定" forState:UIControlStateNormal];
        [_rightBtn setImage:IMAGE_NAMED(@"knb_icon_right") forState:UIControlStateNormal];
...
- (UIView *)lineView {
    if (!_lineView) {
        _lineView = [[UIView alloc]initWithFrame:CGRectMake(0, kTopViewHeight, self.alertView.frame.size.width, 5)];
